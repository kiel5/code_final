...
#include "sensor.h"
#include "input_iot.h"
static const char *TAG_pcnt = "sensor";

int pulse_count = 0;
static QueueHandle_t qPulse;

void sensor_callback()
{
    // ESP_ERROR_CHECK(pcnt_unit_get_count(pcnt_unit, &pulse_count));
    // ESP_LOGI(TAG_pcnt,"hello");
    //     litreClicks++;
...
void pcnt_init(void)
{
	...
	ESP_LOGI(TAG_pcnt,"hello1");
    while(1){
        if(pulse_count == 15)
        {
        ESP_LOGI(TAG_pcnt, "dcmmmmmmm");
        pulse_count = 0;
        }
    }

    // ESP_LOGI(TAG_pcnt, "install pcnt unit");
    // static pcnt_unit_config_t unit_config =                          // config unit
    // {                  
