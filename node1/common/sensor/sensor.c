...
void task_report_pin(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        if (capacity == -5)
        {
            ...
            cJSON_AddStringToObject(data, "Capacity_battery1", "fail");
        ...
        else
        {
            ...
            ESP_LOGI(TAG, "capacity battery1: %d", capacity);
            ...
            //-------------publish mqtt---------------//
            cJSON *data = cJSON_CreateObject();
            cJSON_AddNumberToObject(data, "Capacity_battery1", capacity);
            char *buffer = cJSON_Print(data);
            cJSON_Delete(data);
            int msg_id = mqtt_publish(buffer, strlen(buffer));
...
void task_reportFlow(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        cJSON_AddStringToObject(data, "Litres1", valuelits);
        cJSON_AddStringToObject(data, "Rate1", valuerate);
