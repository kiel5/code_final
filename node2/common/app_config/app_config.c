                          ...
                          int32_t event_id, void *event_data)
{
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        oled_display_text("disconnect_wf", 0, strlen("disconnect_wf"));
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        oled_display_text("wifi_connected", 0, strlen("wifi_connected"));
        s_retry_num = 0;
    }
...
void app_config(void)
{
    ...
    if (provisioned)
    {
        ...
        ESP_LOGI(TAG_config, "connect to ");
    ...
    if (!provisioned) // chưa cấu hình
    {
        ...
        else if (provision_type == PROVISION_ACCESSPOINT)
        {
            while (1)
            {
                ...
                xEventGroupWaitBits(s_wifi_event_group, HTTP_CONFIG_DONE_BIT, true, true, portMAX_DELAY);
                ...
                ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
                ESP_ERROR_CHECK(esp_wifi_start());
                ESP_LOGI(TAG_ap_config, "config done");
                EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, false, false, portMAX_DELAY);
                if (bits & WIFI_CONNECTED_BIT)
                {
                    break;
