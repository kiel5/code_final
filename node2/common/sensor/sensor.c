...
void task_report_pin(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        if (capacity == -5)
        {
            ...
            cJSON_AddStringToObject(data, "Capacity_battery2", "fail");
        ...
        else
        {
            ...
            ESP_LOGI(TAG, "capacity battery2: %d", capacity);
            char str_capacity[14] = "";
            sprintf(str_capacity, "node2_pin  %d%%", capacity);
            oled_display_text(str_capacity, 3, strlen(str_capacity));

            //-------------publish mqtt---------------//
            cJSON *data = cJSON_CreateObject();
            cJSON_AddNumberToObject(data, "Capacity_battery2", capacity);
            char *buffer = cJSON_Print(data);
            cJSON_Delete(data);
            int msg_id = mqtt_publish(buffer, strlen(buffer));
...
void task_reportFlow(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        cJSON_AddStringToObject(data, "device", "node2");
        cJSON_AddStringToObject(data, "Litres2", valuelits);
        cJSON_AddStringToObject(data, "Rate2", valuerate);
        ...
        sprintf(str_value_lits, "node2_lits: %0.2f", report.lits);
        sprintf(str_value_rate, "node2_rate: %0.2f", report.rate);
...
void task_persist(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (flag_button)
        {
            ...
            char str_value_lits[17] = "";
            float x = 0;
            sprintf(str_value_lits, "node2_lits: %0.2f", x);
            sprintf(str_value_rate, "node2_rate: %0.2f", x);
            // oled_display_text("devide: node1", 0, strlen("devide: node1"));
            oled_display_text(str_value_rate, 1, strlen(str_value_rate));
            oled_display_text(str_value_lits, 2, strlen(str_value_lits));
            flag_button = false;
